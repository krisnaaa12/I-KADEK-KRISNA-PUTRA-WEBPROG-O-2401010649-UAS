...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_FILES['photo']['error'] === 0) {
        ...
        $photo_name = $villa['photo'];
    }
    
    // Jika tidak ada error, update database
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE villas SET name = ?, location = ?, price_per_night = ?, description = ?, photo = ? WHERE id = ?");
            $stmt->execute([$name, $location, $price, $description, $photo_name, $id]);
            
            header('Location: ../admin/villas.php');
            exit;
        } catch (PDOException $e) {
            $error = "Gagal mengupdate villa: " . $e->getMessage();
        }
    }
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Edit Villa - Luxury Villas</title>
...
<body>
    <div class="container">
        <div class="form-container">
            <h2 class="form-title">Edit Villa</h2>
            ...
            <form method="POST" enctype="multipart/form-data" id="villaForm">
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" class="form-control" value="<?= isset($_POST['name']) ? htmlspecialchars($_POST['name']) : htmlspecialchars($villa['name']) ?>" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="location" id="location" class="form-control" value="<?= isset($_POST['location']) ? htmlspecialchars($_POST['location']) : htmlspecialchars($villa['location']) ?>" required>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="price_per_night" id="price_per_night" class="form-control" value="<?= isset($_POST['price_per_night']) ? htmlspecialchars($_POST['price_per_night']) : htmlspecialchars($villa['price_per_night']) ?>" required>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" id="description" class="form-control" required><?= isset($_POST['description']) ? htmlspecialchars($_POST['description']) : htmlspecialchars($villa['description']) ?></textarea>
                ...
                <div class="mb-4">
                    ...
                    <img src="../uploads/<?= $villa['photo'] ?>" class="current-image" alt="Foto Villa Saat Ini">
